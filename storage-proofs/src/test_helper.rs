...
pub fn fake_drgpoprep_proof<R: Rng>(
    ...
    let (data_node_path, data_root) =
        random_merkle_path_with_value::<_, PedersenHasher>(rng, tree_depth, &data_node.into(), 0);
    ...
    let (remaining_path, replica_root) = random_merkle_path_with_value::<_, PedersenHasher>(
        rng,
        remaining_depth,
        &subtree_root.into(),
        remaining_depth,
...
pub fn random_merkle_path_with_value<R: Rng, H: Hasher>(
    rng: &mut R,
    tree_depth: usize,
    value: &H::Domain,
    offset: usize,
) -> (Vec<Option<(H::Domain, bool)>>, H::Domain) {
    let auth_path: Vec<Option<(H::Domain, bool)>> = vec![Some((rng.gen(), rng.gen())); tree_depth];

    let mut cur = *value; // if offset == 0 {
    ...
    for (i, p) in auth_path.clone().into_iter().enumerate() {
        ...
        // .into_xy()
        // .0;
        cur = H::Function::hash_node(lhs, rhs, i);
    }

    (auth_path, cur)
}

pub fn random_merkle_path<R: Rng, H: Hasher>(
    rng: &mut R,
    tree_depth: usize,
) -> (Vec<Option<(H::Domain, bool)>>, H::Domain, H::Domain) {
    let value: H::Domain = rng.gen();

    let (path, root) = random_merkle_path_with_value::<_, H>(rng, tree_depth, &value, 0);
